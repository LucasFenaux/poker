...
class PPO(OnPolicyAlgorithm):
    ...
    def mini_batch_update(self, batch_states, batch_rewards, batch_actions, batch_rnn_states=None, sample_weights=None,
        ...
        for i in range(self.sgd_steps):
            ...
            for start_idx in range(0, batch_size, self.mini_batch_size):
                ...
                action_dist, bet_sizing_dist = self.get_model_policy(self.network, mini_batch_states,
                                                                     mini_batch_rnn_states)
    ...
    def full_batch_update(self, batch_states, batch_rewards, batch_actions, batch_rnn_states=None, sample_weights=None,
                          *args,
                          **kwargs):
        batch_size = len(batch_rewards)
    ...
    def update(self, batch_states, batch_rewards, batch_actions, batch_rnn_states=None, sample_weights=None, *args,
        ...
        if self.update_mode == "full_batch":
            metrics = self.full_batch_update(batch_states, batch_rewards, batch_actions, batch_rnn_states,
                                             sample_weights,
                                             *args, **kwargs)
        elif self.update_mode == "mini_batch":
            metrics = self.mini_batch_update(batch_states, batch_rewards, batch_actions, batch_rnn_states,
                                             sample_weights,
                                             *args, **kwargs)
        else:
...
class PPOInferenceWrapper(InferenceWrapper):
    ...
    def get_action(self, state: (pokerkit.State, int), hand_hidden: torch.Tensor = None,
                   game_hidden: torch.Tensor = None):
        action_dist, bet_sizing_dist = self.get_model_policy(self.network, state)
